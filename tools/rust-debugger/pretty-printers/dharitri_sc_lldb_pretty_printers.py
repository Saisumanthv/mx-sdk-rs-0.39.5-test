...
DEBUG_API_TYPE = "drt_chain_vm::tx_mock::tx_context_ref::TxContextRef"
ANY_NUMBER = "[0-9]+"
ANY_TYPE = ".*"
...
# 2. Numbat wasm - Managed basic types
MOD_PATH = "drt_sc::types::managed::basic"
...
# 3. Numbat wasm - Managed wrapped types
MOD_PATH = "drt_sc::types::managed::wrapped"
...
# 4. Numbat wasm - Managed multi value types

# 5. Numbat wasm - heap
MOD_PATH = "drt_sc::types::heap"
...
MOD_PATH = "drt_sc_codec::multi_types"
...
class DcdtTokenPayment(ManagedVecItem, ManagedType):
...
def __lldb_init_module(debugger: SBDebugger, dict):
    ...
    for rust_type, handler_class in NUMBAT_WASM_TYPE_HANDLERS:
        ...
        summary_command = f'type summary add -x "^{rust_type}$" -F {python_module_name}.{summary_function_name} --category drt-sc'
        debugger.HandleCommand(summary_command)
        # print(f"Registered: {summary_command}")

    # Enable categories
    debugger.HandleCommand('type category enable drt-sc')
