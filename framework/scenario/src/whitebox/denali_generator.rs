...
use drt_chain_vm::world_mock::{AccountData, BlockInfo};

pub(crate) struct DenaliGenerator {
    scenario: ScenarioRaw,
    current_tx_id: u64,
}

impl DenaliGenerator {
    pub fn new() -> Self {
        Self {
            scenario: ScenarioRaw {
                check_gas: None,
    ...
    pub fn write_denali_output(self, file_path: &str) {
        let buf = Vec::new();
...
impl DenaliGenerator {
    ...
    pub fn create_tx(&mut self, tx: &ScCallDenali, opt_expect: Option<&TxExpectDenali>) {
    ...
    pub fn create_query(&mut self, query: &ScQueryDenali, opt_expect: Option<&TxExpectDenali>) {
