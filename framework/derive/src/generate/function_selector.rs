...
pub fn generate_function_selector_body(contract: &ContractTrait) -> proc_macro2::TokenStream {
    let match_arms: Vec<proc_macro2::TokenStream> = contract
        ...
        .filter_map(|m| match &m.public_role {
            PublicRole::Init(_) => Some(endpoint_match_arm(
                m,
                "init",
                quote! { if !<Self::Api as drt_sc::api::VMApi>::external_view_init_override() },
            )),
    ...
    quote! {
        if match fn_name {
            ...
            "init" if <Self::Api as drt_sc::api::VMApi>::external_view_init_override() => {
                drt_sc::external_view_contract::external_view_contract_constructor::<Self::Api>();
                return true;
            },
            #(#match_arms)*
            other => false
