...
fn generate_endpoint_snippet(
    ...
    quote! {
        let mut endpoint_abi = drt_sc::abi::EndpointAbi{
            docs: &[ #(#endpoint_docs),* ],
            name: #endpoint_name,
            ...
            inputs: drt_sc::types::heap::Vec::new(),
            outputs: drt_sc::types::heap::Vec::new(),
            labels: &[ #(#label_names),* ],
        };
        #(#input_snippets)*
        #output_snippet
...
fn generate_event_snippet(m: &Method, event_name: &str) -> proc_macro2::TokenStream {
    ...
    quote! {
        let mut event_abi = drt_sc::abi::EventAbi{
            docs: &[ #(#event_docs),* ],
...
fn generate_supertrait_snippets(contract: &ContractTrait) -> Vec<proc_macro2::TokenStream> {
    contract
			...
			.map(|supertrait| {
				...
				quote! {
					contract_abi.coalesce(<#module_path AbiProvider as drt_sc::contract_base::ContractAbiProvider>::abi());
				}
			})
			.collect()
}
...
fn generate_abi_method_body(
    ...
    quote! {
        let mut contract_abi = drt_sc::abi::ContractAbi {
            build_info: drt_sc::abi::BuildInfoAbi {
            ...
            docs: &[ #(#contract_docs),* ],
            name: #contract_name,
            constructors: drt_sc::types::heap::Vec::new(),
            endpoints: drt_sc::types::heap::Vec::new(),
            promise_callbacks: drt_sc::types::heap::Vec::new(),
            events: drt_sc::types::heap::Vec::new(),
            has_callback: #has_callbacks,
            type_descriptions: <drt_sc::abi::TypeDescriptionContainerImpl as drt_sc::abi::TypeDescriptionContainer>::new(),
        };
        #(#endpoint_snippets)*
        #(#event_snippets)*
        #(#supertrait_snippets)*
