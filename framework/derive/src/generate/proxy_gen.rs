...
pub fn generate_proxy_endpoint(m: &Method, endpoint_name: String) -> proc_macro2::TokenStream {
    ...
    let mut token_expr =
        quote! { drt_sc::types::RewaOrDcdtTokenIdentifier::<Self::Api>::rewa() };
    let mut nonce_count = 0;
    let mut nonce_expr = quote! { 0u64 };
    let mut payment_count = 0;
    let mut payment_expr = quote! { drt_sc::types::BigUint::<Self::Api>::zero() };
    let mut multi_count = 0;
    let mut multi_expr = quote! { drt_sc::types::ManagedVec::<Self::Api, DcdtTokenPayment<Self::Api>>::new() };
        ...
        match &arg.metadata.payment {
            ArgPaymentMetadata::NotPayment => {
                ...
                arg_push_snippets.push(quote! {
                    drt_sc::types::ContractCall::proxy_arg(&mut ___contract_call___, &#pat);
                });
            },
    ...
    if token_count > 0 || nonce_count > 0 || payment_count > 0 {
        ...
        if token_count == 0 && nonce_count == 0 {
            contract_call_type = quote! { drt_sc::types::ContractCallWithRewa };
            contract_call_init = quote! {
                let mut ___contract_call___ = drt_sc::types::ContractCallWithRewa::new(
                    ___address___,
                    #endpoint_name,
                    #payment_expr,
                );
            ...
            contract_call_type = quote! { drt_sc::types::ContractCallWithRewaOrSingleDcdt };
            contract_call_init = quote! {
                let mut ___contract_call___ = drt_sc::types::ContractCallWithRewaOrSingleDcdt::new(
                    ___address___,
                    #endpoint_name,
                    #token_expr,
                    #nonce_expr,
        ...
        contract_call_type = quote! { drt_sc::types::ContractCallWithMultiDcdt };
        contract_call_init = quote! {
            let mut ___contract_call___ = drt_sc::types::ContractCallWithMultiDcdt::new(
                ___address___,
                #endpoint_name,
                #multi_expr,
            );
        ...
        contract_call_type = quote! { drt_sc::types::ContractCallNoPayment };
        contract_call_init = quote! {
            let mut ___contract_call___ = drt_sc::types::ContractCallNoPayment::new(
                ___address___,
                #endpoint_name,
            );
        };
...
pub fn generate_proxy_deploy(init_method: &Method) -> proc_macro2::TokenStream {
    let msig =
        generate_proxy_method_sig(init_method, quote! { drt_sc::types::ContractDeploy });
    ...
    let sig = quote! {
        ...
        #msig {
            ...
            let mut ___contract_deploy___ = drt_sc::types::new_contract_deploy(
                ___opt_address___,
            );
            #(#arg_push_snippets)*
