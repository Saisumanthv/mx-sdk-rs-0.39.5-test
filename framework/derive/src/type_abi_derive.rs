...
fn field_snippet(index: usize, field: &syn::Field) -> proc_macro2::TokenStream {
    ...
    quote! {
        field_descriptions.push(drt_sc::abi::StructFieldDescription {
            docs: &[ #(#field_docs),* ],
            name: #field_name_str,
...
pub fn type_abi_derive(ast: &syn::DeriveInput) -> TokenStream {
    ...
    let type_description_impl = match &ast.data {
        syn::Data::Struct(data_struct) => {
            ...
            quote! {
                fn provide_type_descriptions<TDC: drt_sc::abi::TypeDescriptionContainer>(accumulator: &mut TDC) {
                    let type_name = Self::type_name();
                    if !accumulator.contains_type(&type_name) {
                        accumulator.reserve_type_name(type_name.clone());
                        let mut field_descriptions = drt_sc::types::heap::Vec::new();
                        #(#struct_field_snippets)*
                        accumulator.insert(
                            type_name.clone(),
                            drt_sc::abi::TypeDescription {
                                docs: &[ #(#type_docs),* ],
                                name: type_name,
                                contents: drt_sc::abi::TypeContents::Struct(field_descriptions),
                            },
                        );
                    }
                }
        ...
        syn::Data::Enum(data_enum) => {
            let enum_variant_snippets: Vec<proc_macro2::TokenStream> = data_enum
                ...
                .map(|(variant_index, variant)| {
                    ...
                    quote! {
                        ...
                        variant_descriptions.push(drt_sc::abi::EnumVariantDescription {
                            docs: &[ #(#variant_docs),* ],
            ...
            quote! {
                fn provide_type_descriptions<TDC: drt_sc::abi::TypeDescriptionContainer>(accumulator: &mut TDC) {
                    ...
                    if !accumulator.contains_type(&type_name) {
                        accumulator.reserve_type_name(type_name.clone());
                        let mut variant_descriptions = drt_sc::types::heap::Vec::new();
                        ...
                        accumulator.insert(
                            ...
                            drt_sc::abi::TypeDescription {
                                docs: &[ #(#type_docs),* ],
                                name: type_name,
                                contents: drt_sc::abi::TypeContents::Enum(variant_descriptions),
                            },
                        );
                    }
                }
    ...
    let type_abi_impl = quote! {
        impl #impl_generics drt_sc::abi::TypeAbi for #name #ty_generics #where_clause {
            fn type_name() -> drt_sc::abi::TypeName {
                #name_str.into()
            }
