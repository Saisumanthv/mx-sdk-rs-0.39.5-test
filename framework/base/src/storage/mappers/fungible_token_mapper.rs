...
use crate::{
    ...
    dcdt::{DCDTSystemSmartContractProxy, FungibleTokenProperties},
    storage::StorageKey,
    types::{
        BigUint, CallbackClosure, ContractCall, DcdtTokenPayment, DcdtTokenType, ManagedAddress,
    ...
    SA: StorageMapperApi + CallTypeApi,
{
    ...
    /// If you want to use default callbacks, import the default_issue_callbacks::DefaultIssueCallbacksModule from drt-sc-modules
    ...
    pub fn mint(&self, amount: BigUint<SA>) -> DcdtTokenPayment<SA> {
        let send_wrapper = SendWrapper::<SA>::new();
        let token_id = self.get_token_id();

        send_wrapper.dcdt_local_mint(&token_id, 0, &amount);

        DcdtTokenPayment::new(token_id, 0, amount)
    }
    ...
    pub fn burn(&self, amount: &BigUint<SA>) {
        ...
        send_wrapper.dcdt_local_burn(token_id, 0, amount);
    }
    ...
    pub fn get_balance(&self) -> BigUint<SA> {
        ...
        b_wrapper.get_dcdt_balance(&own_sc_address, token_id, 0)
    }

    fn send_payment(&self, to: &ManagedAddress<SA>, payment: &DcdtTokenPayment<SA>) {
        let send_wrapper = SendWrapper::<SA>::new();
        send_wrapper.direct_dcdt(to, &payment.token_identifier, 0, &payment.amount);
    }
}
