// Note: Simple macros cannot be placed in drt-sc-derive,
...
macro_rules! sc_error {
    ($s:expr) => {
        drt_sc::types::SCResult::Err(drt_sc::types::StaticSCError::from($s)).into()
    };
}
...
///
/// ```rust
/// # use drt_sc::require_old;
/// # use drt_sc::types::{*, SCResult::Ok};
/// # pub trait ExampleContract: drt_sc::contract_base::ContractBase
/// # {
...
macro_rules! require_old {
    ($expression:expr, $error_msg:expr) => {
        if (!($expression)) {
            return drt_sc::sc_error!($error_msg);
        }
    };
}
...
macro_rules! sc_panic {
    ($msg:tt, $($arg:expr),+ $(,)?) => {{
        let mut ___buffer___ =
            drt_sc::types::ManagedBufferCachedBuilder::<Self::Api>::new_from_slice(&[]);
        drt_sc::derive::format_receiver_args!(___buffer___, $msg, $($arg),+);
        drt_sc::contract_base::ErrorHelper::<Self::Api>::signal_error_with_message(___buffer___.into_managed_buffer());
    }};
    ($msg:expr $(,)?) => {
        drt_sc::contract_base::ErrorHelper::<Self::Api>::signal_error_with_message($msg);
    };
}
...
///
/// ```rust
/// # use drt_sc::{types::ManagedBuffer, require};
/// # pub trait ExampleContract: drt_sc::contract_base::ContractBase
/// # {
...
macro_rules! sc_print {
    ($msg:tt, $($arg:expr),* $(,)?) => {{
        let mut ___buffer___ =
            <<Self::Api as drt_sc::api::PrintApi>::PrintApiImpl as drt_sc::api::PrintApiImpl>::Buffer::default();
        drt_sc::derive::format_receiver_args!(___buffer___, $msg, $($arg),*);
        <<Self::Api as drt_sc::api::PrintApi>::PrintApiImpl as drt_sc::api::PrintApiImpl>::print_buffer(
            &<Self::Api as drt_sc::api::PrintApi>::print_api_impl(),
            ___buffer___,
        );
    }};
}
...
macro_rules! sc_format {
    ...
    ($msg:expr $(,)?) => {{
        drt_sc::types::ManagedBuffer::new_from_bytes($msg.as_bytes())
    }};
}
...
macro_rules! sc_try {
    ($s:expr) => {
        match $s {
            ...
            drt_sc::types::SCResult::Err(e) => {
                return drt_sc::types::SCResult::Err(e);
            },
        }
    };
}
