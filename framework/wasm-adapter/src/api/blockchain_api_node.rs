...
use drt_sc::{
    ...
    types::{
        ...
        BigUint, DcdtTokenData, DcdtTokenType, ManagedAddress, ManagedBuffer, ManagedType,
...
impl BlockchainApiImpl for VmApiImpl {
    ...
    fn load_dcdt_token_data_unmanaged<M: ManagedTypeApi>(
        ...
        use drt_sc::{api::BigIntApi, types::heap::BoxedBytes};
    ...
    fn check_dcdt_frozen(
        ...
        unsafe { managedIsDCDTFrozen(address_handle, token_id_handle, nonce as i64) > 0 }
    }

    fn check_dcdt_paused(&self, token_id_handle: Self::ManagedBufferHandle) -> bool {
        unsafe { managedIsDCDTPaused(token_id_handle) > 0 }
    }

    fn check_dcdt_limited_transfer(&self, token_id_handle: Self::ManagedBufferHandle) -> bool {
        unsafe { managedIsDCDTLimitedTransfer(token_id_handle) > 0 }
    }

    fn load_dcdt_local_roles(
        &self,
        token_id_handle: Self::ManagedBufferHandle,
    ) -> drt_sc::types::DcdtLocalRoleFlags {
        unsafe {
            drt_sc::types::DcdtLocalRoleFlags::from_bits_unchecked(getDCDTLocalRoles(
                token_id_handle,
            ) as u64)
        }
    }
