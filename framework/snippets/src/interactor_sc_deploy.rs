use crate::{denali_to_moars_address, Interactor, InteractorResult};
use log::info;
use drt_sc_scenario::{
    drt_sc::codec::{CodecFrom, TopEncodeMulti},
    scenario_model::{ScDeployStep, TypedScDeploy},
};
use drt_sdk::data::{address::Address as MoarsAddress, transaction::Transaction};
...
impl Interactor {
    fn sc_deploy_to_tx(&self, sc_deploy_step: &ScDeployStep) -> Transaction {
        Transaction {
            ...
            sender: denali_to_moars_address(&sc_deploy_step.tx.from),
