...
fn v_0_39_prepare_meta(sc_crate_path: &Path) {
    ...
    print_cargo_dep_add(cargo_toml_path.as_path(), "drt-sc-meta");
    let mut meta_dep = Table::new();
    meta_dep.insert("version".to_string(), Value::String("0.39.0".to_string()));
    deps.insert("drt-sc-meta".to_string(), Value::Table(meta_dep));
...
fn v_0_39_replace_in_files(sc_crate_path: &Path) {
    replace_in_files(
        ...
        "*Cargo.toml",
        &[
            Query::substring("numbat-wasm-debug", "drt-sc-scenario"),
            Query::substring("numbat-wasm-modules", "drt-sc-modules"),
            Query::substring("numbat-wasm-node", "drt-sc-wasm-adapter"),
            Query::substring("numbat-wasm", "drt-sc"),
        ][..],
    );
    ...
    replace_in_files(
        ...
        &[
            ...
            Query::substring("numbat_wasm_debug", "drt_sc_scenario"),
            Query::substring("numbat_wasm_modules", "drt_sc_modules"),
            Query::substring("numbat_wasm_node", "drt_sc_wasm_adapter"),
            Query::substring("numbat_wasm", "drt_sc"),
