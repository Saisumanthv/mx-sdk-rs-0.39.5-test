use drt_sc::types::{RewaOrDcdtTokenIdentifier, ManagedVec};
use drt_sc_modules::staking::StakingModule;
use drt_sc_scenario::{
...
fn staking_module_test() {
    ...
    b_mock.set_dcdt_balance(&alice, STAKING_TOKEN_ID, &rust_biguint!(INITIAL_BALANCE));
    b_mock.set_dcdt_balance(&bob, STAKING_TOKEN_ID, &rust_biguint!(INITIAL_BALANCE));
    b_mock.set_dcdt_balance(&carol, STAKING_TOKEN_ID, &rust_biguint!(INITIAL_BALANCE));
    b_mock.set_dcdt_balance(&eve, STAKING_TOKEN_ID, &rust_biguint!(INITIAL_BALANCE));
    ...
    b_mock
        .execute_tx(&owner, &staking_sc, &rust_zero, |sc| {
            ...
            sc.init_staking_module(
                &RewaOrDcdtTokenIdentifier::dcdt(managed_token_id!(STAKING_TOKEN_ID)),
    ...
    b_mock
        ...
        .assert_ok();
    b_mock.check_dcdt_balance(&alice, STAKING_TOKEN_ID, &rust_biguint!(1_000_000));
