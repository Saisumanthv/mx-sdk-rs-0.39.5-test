use drt_sc::types::{
    Address, DcdtLocalRole, DcdtTokenPayment, ManagedArgBuffer, ManagedVec, MultiValueEncoded,
};
use drt_sc_modules::transfer_role_proxy::TransferRoleProxyModule;
use drt_sc_scenario::{
...
fn test_transfer_role_module() {
    ...
    b_mock.set_dcdt_balance(&user, TRANSFER_TOKEN_ID, &rust_biguint!(1_000));
    ...
    b_mock.check_dcdt_balance(&user, TRANSFER_TOKEN_ID, &rust_biguint!(900));
    b_mock.check_dcdt_balance(&owner, TRANSFER_TOKEN_ID, &rust_biguint!(100));
    ...
    b_mock.check_dcdt_balance(&user, TRANSFER_TOKEN_ID, &rust_biguint!(800));
