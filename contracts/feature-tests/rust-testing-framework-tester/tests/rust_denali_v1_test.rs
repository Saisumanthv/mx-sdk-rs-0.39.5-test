...
const TEST_DCDT_OUTPUT_PATH: &str = "test_dcdt_generation.scen.json";
...
fn test_sc_payment_ok() {
    ...
    wrapper
        .execute_tx(&caller_addr, &sc_wrapper, &rust_biguint!(1_000), |sc| {
            let actual_payment = sc.receive_rewa();
            let expected_payment = managed_biguint!(1_000);
            assert_eq!(actual_payment, expected_payment);
        })
        .assert_ok();

    wrapper.check_rewa_balance(&caller_addr, &rust_biguint!(0));
    wrapper.check_rewa_balance(sc_wrapper.address_ref(), &rust_biguint!(3_000));
}
...
fn test_sc_payment_reverted() {
    ...
    wrapper.check_rewa_balance(&caller_addr, &rust_biguint!(1_000));
    wrapper.check_rewa_balance(sc_wrapper.address_ref(), &rust_biguint!(2_000));
}
...
fn test_sc_half_payment() {
    ...
    wrapper
        .execute_tx(&caller_addr, &sc_wrapper, &rust_biguint!(1_000), |sc| {
            sc.recieve_rewa_half();
        })
        .assert_ok();

    wrapper.check_rewa_balance(&caller_addr, &rust_biguint!(500));
    wrapper.check_rewa_balance(sc_wrapper.address_ref(), &rust_biguint!(2_500));
}

#[test]
fn test_dcdt_balance() {
    let mut wrapper = BlockchainStateWrapper::new();
    let sc_wrapper = wrapper.create_sc_account(
        &rust_biguint!(0),
        None,
    ...
    wrapper.set_dcdt_balance(sc_wrapper.address_ref(), token_id, &rust_biguint!(1_000));
    wrapper.check_dcdt_balance(sc_wrapper.address_ref(), token_id, &rust_biguint!(1_000));

    wrapper
        .execute_query(&sc_wrapper, |sc| {
            let managed_id = managed_token_id!(token_id);

            let actual_balance = sc.get_dcdt_balance(managed_id, 0);
...
fn test_dcdt_payment_ok() {
    ...
    wrapper.set_dcdt_balance(&caller_addr, token_id, &rust_biguint!(1_000));
    wrapper.set_dcdt_balance(sc_wrapper.address_ref(), token_id, &rust_biguint!(2_000));
    ...
    wrapper
        .execute_dcdt_transfer(
            ...
            &rust_biguint!(1_000),
            |sc| {
                let (actual_token_id, actual_payment) = sc.receive_dcdt();
                let expected_payment = managed_biguint!(1_000);
    ...
    wrapper.check_dcdt_balance(&caller_addr, token_id, &rust_zero);
    wrapper.check_dcdt_balance(sc_wrapper.address_ref(), token_id, &rust_biguint!(3_000));
}

#[test]
fn test_dcdt_payment_reverted() {
    let mut wrapper = BlockchainStateWrapper::new();
    let rust_zero = rust_biguint!(0);
    ...
    wrapper.check_dcdt_balance(&caller_addr, token_id, &rust_biguint!(1_000));
    wrapper.check_dcdt_balance(sc_wrapper.address_ref(), token_id, &rust_biguint!(2_000));
}
...
fn test_nft_balance() {
    ...
    wrapper
        .execute_query(&sc_wrapper, |sc| {
            ...
            let actual_balance = sc.get_dcdt_balance(managed_id, nft_nonce);
            let expected_balance = managed_biguint!(1_000);
            assert_eq!(expected_balance, actual_balance);
        })
        .assert_ok();
...
fn test_sc_dcdt_mint_burn() {
    ...
    wrapper
        .execute_tx(&caller_addr, &sc_wrapper, &rust_biguint!(0), |sc| {
            let managed_id = managed_token_id!(token_id);
            let managed_amt = managed_biguint!(400);
            sc.mint_dcdt(managed_id, 0, managed_amt);
        })
        .assert_ok();

    wrapper.check_dcdt_balance(sc_wrapper.address_ref(), token_id, &rust_biguint!(400));

    wrapper
        .execute_tx(&caller_addr, &sc_wrapper, &rust_biguint!(0), |sc| {
            let managed_id = managed_token_id!(token_id);
            let managed_amt = managed_biguint!(100);
            sc.burn_dcdt(managed_id, 0, managed_amt);
        })
        .assert_ok();

    wrapper.check_dcdt_balance(sc_wrapper.address_ref(), token_id, &rust_biguint!(300));
}
...
fn test_sc_nft() {
    ...
    wrapper
        .execute_tx(&caller_addr, &sc_wrapper, &rust_biguint!(0), |sc| {
            let managed_id = managed_token_id!(token_id);
            let managed_amt = managed_biguint!(100);
            sc.mint_dcdt(managed_id, 1, managed_amt);
        })
        .assert_ok();
    ...
    wrapper
        .execute_tx(&caller_addr, &sc_wrapper, &rust_biguint!(0), |sc| {
            ...
            sc.burn_dcdt(managed_id, 2, managed_amt);
        })
        .assert_ok();
...
fn test_dcdt_multi_transfer() {
    ...
    wrapper.set_dcdt_balance(&caller_addr, token_id_1, &rust_biguint!(100));
    wrapper.set_nft_balance(
        &caller_addr,
        token_id_2,
        nft_nonce,
    ...
    wrapper
        .execute_dcdt_multi_transfer(&caller_addr, &sc_wrapper, &transfers, |sc| {
            let mut expected_transfers = Vec::new();
            expected_transfers.push(DcdtTokenPayment::new(
            ...
            let actual_transfers = sc.receive_multi_dcdt().into_vec();
            assert_eq!(
    ...
    wrapper.check_dcdt_balance(sc_wrapper.address_ref(), token_id_1, &rust_biguint!(100));
...
fn test_query() {
    ...
    let _ = wrapper.execute_query(&sc_wrapper, |sc| {
        let actual_balance = sc.get_rewa_balance();
...
fn test_wrapper_getters() {
    let mut wrapper = BlockchainStateWrapper::new();
    let rewa_balance = rust_biguint!(1_000);

    let dcdt_token_id = b"DCDT-123456";
    let dcdt_balance = rust_biguint!(100);
    ...
    let user_addr = wrapper.create_user_account(&rewa_balance);
    wrapper.set_dcdt_balance(&user_addr, dcdt_token_id, &dcdt_balance);
    wrapper.set_nft_balance(
        &user_addr,
        nft_token_id,
        nft_nonce,
    ...
    let actual_rewa_balance = wrapper.get_rewa_balance(&user_addr);
    let actual_dcdt_balance = wrapper.get_dcdt_balance(&user_addr, dcdt_token_id, 0);
    let actual_nft_balance = wrapper.get_dcdt_balance(&user_addr, nft_token_id, nft_nonce);
    let actual_attributes = wrapper
        .get_nft_attributes::<NftDummyAttributes>(&user_addr, nft_token_id, nft_nonce)
        .unwrap();

    assert_eq!(rewa_balance, actual_rewa_balance);
    assert_eq!(dcdt_balance, actual_dcdt_balance);
...
fn test_managed_values_standalone_consistency() {
    ...
    let foo_token = TokenIdentifier::<DebugApi>::from_dcdt_bytes(b"FOO-a1a1a1");
    blockchain_wrapper
        .execute_query(&basic_features_wrapper, |_sc| {
            let _bar = TokenIdentifier::<DebugApi>::from_dcdt_bytes(b"BAR-a1a1a1");
            // 'foo' and '_bar' have the same numerical handle value
            // check that the value of 'foo' is taken from the correct context
            assert_eq!(
                foo_token,
                TokenIdentifier::<DebugApi>::from_dcdt_bytes(b"FOO-a1a1a1")
            );
        })
        .assert_ok();
}
...
fn test_back_and_forth_transfers() {
    ...
    wrapper.set_dcdt_balance(&user, &first_token_id[..], &first_token_amount);
    wrapper.set_dcdt_balance(&user, &second_token_id[..], &second_token_amount);
    ...
    wrapper
        .execute_dcdt_multi_transfer(&user, &forwarder_wrapper, &transfers, |sc| {
            let mut managed_payments = ManagedVec::new();
            managed_payments.push(DcdtTokenPayment::new(
...
fn dump_state_single_test() {
    ...
    wrapper.set_rewa_balance(sc_wrapper.address_ref(), &rust_biguint!(444));
    wrapper.set_dcdt_balance(
        sc_wrapper.address_ref(),
        fungible_token_id,
        &rust_biguint!(1_000),
