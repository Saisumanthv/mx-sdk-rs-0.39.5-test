...
fn price_agg_submit_test() {
    let mut pa_setup = PriceAggSetup::new(drt_price_aggregator_sc::contract_obj);
    let current_timestamp = 100;
    let oracles = pa_setup.oracles.clone();

    // configure the number of decimals
    pa_setup.set_pair_decimals(REWA_TICKER, USD_TICKER, DECIMALS);
...
fn price_agg_submit_round_ok_test() {
    ...
    pa_setup
        ...
        .execute_query(&pa_setup.price_agg, |sc| {
            let result = sc
                .latest_price_feed(managed_buffer!(REWA_TICKER), managed_buffer!(USD_TICKER))
                .unwrap();
