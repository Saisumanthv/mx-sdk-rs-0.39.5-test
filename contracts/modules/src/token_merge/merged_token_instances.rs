...
pub type InstanceArray<M> = ArrayVec<DcdtTokenPayment<M>, MAX_MERGED_TOKENS>;
...
impl<M: ManagedTypeApi> MergedTokenInstances<M> {
    ...
    pub fn add_or_update_instance(&mut self, new_instance: DcdtTokenPayment<M>) {
        let search_result =
    ...
    pub fn deduct_balance_for_instance(&mut self, tokens_to_deduct: &DcdtTokenPayment<M>) {
