    ...
    + crate::pause::PauseModule
{
    fn merge_tokens<AttributesCreator: MergedTokenAttributesCreator<ScType = Self>>(
        ...
        for token in &payments {
            if token.token_identifier == merged_token_id {
                let token_data = self.blockchain().get_dcdt_token_data(
                    &sc_address,
        ...
        self.send()
            .direct_non_zero_dcdt_payment(&caller, &merged_token_payment);

        merged_token_payment
    }

    fn split_tokens(&self, payments: ManagedVec<DcdtTokenPayment>) -> ManagedVec<DcdtTokenPayment> {
        self.require_not_paused();
    ...
    fn split_token_partial<AttributesCreator: MergedTokenAttributesCreator<ScType = Self>>(
        &self,
        merged_token: DcdtTokenPayment,
        mut tokens_to_remove: ManagedVec<DcdtTokenPayment>,
        attr_creator: &AttributesCreator,
        ...
        let sc_address = self.blockchain().get_sc_address();
        let merged_token_data = self.blockchain().get_dcdt_token_data(
            &sc_address,
