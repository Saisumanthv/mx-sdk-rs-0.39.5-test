...
pub trait MergedTokenSetupModule {
    ...
    fn create_merged_token<AttributesCreator: MergedTokenAttributesCreator<ScType = Self>>(
        ...
        let merged_token_nonce = self.send().dcdt_nft_create(
            &merged_token_id,
            &nft_amount,
            &empty_buffer,
            &royalties,
        ...
        DcdtTokenPayment::new(merged_token_id, merged_token_nonce, nft_amount)
    }
    ...
    fn collect_token_data(
        &self,
        merged_instances: &MergedTokenInstances<Self::Api>,
    ) -> ArrayVec<DcdtTokenData<Self::Api>, MAX_MERGED_TOKENS> {
    ...
    fn require_all_parts_same_creator(
        &self,
        all_token_data: &ArrayVec<DcdtTokenData<Self::Api>, MAX_MERGED_TOKENS>,
    ) {
