...
use drt_sc::contract_base::ManagedSerializer;
...
pub trait UserEndpointsModule: storage::StorageModule + events::EventsModule {
    ...
    where
        T: CurveFunction<Self::Api>
            ...
            + Default,
    {
        let (offered_token, nonce, sell_amount) = self.call_value().single_dcdt().into_tuple();
            ...
            + Default,
    {
        ...
        match requested_nonce {
            OptionalValue::Some(nonce) => {
                self.send()
                    .direct_dcdt(&caller, &requested_token, nonce, &requested_amount);
    ...
    fn send_next_available_tokens(
        ...
        let mut tokens_to_send = ManagedVec::<Self::Api, DcdtTokenPayment<Self::Api>>::new();
        loop {
            ...
            tokens_to_send.push(DcdtTokenPayment::new(token.clone(), nonce, amount_to_send));
