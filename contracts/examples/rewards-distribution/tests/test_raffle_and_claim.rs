...
fn test_raffle_and_claim() {
    ...
    wrapper
        .execute_dcdt_multi_transfer(&alice, &rewards_distribution_sc, &nft_payments, |sc| {
            ...
            let mut reward_tokens: MultiValueEncoded<
                DebugApi,
                MultiValue2<RewaOrDcdtTokenIdentifier<DebugApi>, u64>,
            ...
            for nonce in nft_nonces {
                let was_claimed = sc
                    .was_claimed(raffle_id, &RewaOrDcdtTokenIdentifier::rewa(), 0, nonce)
                    .get();
                assert!(was_claimed);
            }
        })
    ...
    wrapper.check_rewa_balance(&alice, &rust_biguint!(alice_balance_after_claim));
