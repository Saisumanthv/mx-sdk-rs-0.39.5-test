...
use drt_sc_modules::dns::ProxyTrait as _;
use drt_sc_snippets::{
...
const GATEWAY: &str = drt_sc_snippets::moars::blockchain::TESTNET_GATEWAY;
...
async fn main() {
    ...
    match cmd.as_str() {
        ...
        "feed" => state.feed_contract_rewa().await,
...
impl State {
    ...
    async fn deploy(&mut self) {
        let deploy_result: drt_sc_snippets::InteractorResult<()> = self
            .interactor
            .sc_deploy(
                self.multisig
        ...
        let _ = self
            ...
            .transfer(
                TransferStep::new()
                    .from(&self.wallet_address)
                    .to(&self.multisig)
                    .rewa_value("0,050000000000000000"),
            )
            .await;
    }
