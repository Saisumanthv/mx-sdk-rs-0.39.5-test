...
// Deposit 10 REWA from alice to the multisig contract, which we'll use them in the next examples
await multisig.gas(20_000_000).sender(alice).value(Rewa(10)).call.deposit();

// Create a proposal to send 3 REWA from the multisig to eve
// Note that any proposal is automatically signed by alice
var sendId = await multisig.sender(alice).call.proposeSendRewa(eve, Rewa(3));
...
var deployId = await multisig.sender(alice).gas(200_000_000).call.proposeSCDeploy(Rewa(0), await adder.getCode(), false, false, false, formattedDeploy);
...
var addId = await multisig.sender(alice).call.proposeSendDcdt(adder, Rewa(0), formattedAdd);
