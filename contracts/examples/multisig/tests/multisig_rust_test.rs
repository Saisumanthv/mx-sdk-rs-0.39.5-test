...
use drt_sc::types::{BoxedBytes, CodeMetadata, ManagedAddress};
use drt_sc_scenario::{managed_address, managed_biguint, rust_biguint, DebugApi};
...
use crate::multisig_rust_test_setup::{ActionRaw, REWA_TOKEN_ID};
...
fn transfer_execute_to_user_test() {
    ...
    ms_setup
        .call_deposit(REWA_TOKEN_ID, rewa_amount)
        .assert_ok();

    ms_setup.b_mock.check_rewa_balance(
        ms_setup.ms_wrapper.address_ref(),
        &rust_biguint!(rewa_amount),
    );
    ...
    ms_setup
        .b_mock
        .check_rewa_balance(&user_addr, &rust_biguint!(rewa_amount));
}
