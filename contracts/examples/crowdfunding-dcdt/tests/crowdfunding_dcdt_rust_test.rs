...
use drt_sc::types::{Address, RewaOrDcdtTokenIdentifier};
use drt_sc_scenario::{
...
where
    CrowdfundingObjBuilder: 'static + Copy + Fn() -> crowdfunding_dcdt::ContractObj<DebugApi>,
{
    ...
    pub cf_wrapper:
        ContractObjWrapper<crowdfunding_dcdt::ContractObj<DebugApi>, CrowdfundingObjBuilder>,
}
    ...
    CrowdfundingObjBuilder: 'static + Copy + Fn() -> crowdfunding_dcdt::ContractObj<DebugApi>,
{
    ...
    blockchain_wrapper.set_dcdt_balance(&first_user_address, CF_TOKEN_ID, &rust_biguint!(1_000));
    blockchain_wrapper.set_dcdt_balance(&second_user_address, CF_TOKEN_ID, &rust_biguint!(1_000));
...
#[test]
fn init_test() {
    let cf_setup = setup_crowdfunding(crowdfunding_dcdt::contract_obj);
    cf_setup
        .blockchain_wrapper
...
#[test]
fn fund_test() {
    let mut cf_setup = setup_crowdfunding(crowdfunding_dcdt::contract_obj);
    ...
    sc_call.add_dcdt_transfer(CF_TOKEN_ID, 0, &rust_biguint!(1_000));
...
fn test_sc_error() {
    ...
    b_wrapper.set_rewa_balance(user_addr, &rust_biguint!(1_000));
    ...
    let mut sc_call = ScCallDenali::new(user_addr, cf_setup.cf_wrapper.address_ref(), "fund");
    sc_call.add_rewa_value(&rust_biguint!(1_000));
...
fn test_successful_cf() {
    ...
    b_wrapper.check_dcdt_balance(owner, CF_TOKEN_ID, &rust_biguint!(2_000));
    b_wrapper.check_dcdt_balance(first_user, CF_TOKEN_ID, &rust_biguint!(0));
    b_wrapper.check_dcdt_balance(second_user, CF_TOKEN_ID, &rust_biguint!(0));
}
...
fn test_failed_cf() {
    ...
    b_wrapper.check_dcdt_balance(owner, CF_TOKEN_ID, &rust_biguint!(0));
    b_wrapper.check_dcdt_balance(first_user, CF_TOKEN_ID, &rust_biguint!(1_000));
    b_wrapper.check_dcdt_balance(second_user, CF_TOKEN_ID, &rust_biguint!(1_000));
}
