...
use drt_sc_modules::default_issue_callbacks;
...
pub trait FractionalNfts: default_issue_callbacks::DefaultIssueCallbacksModule {
    ...
    fn fractionalize_nft(
        ...
        let original_payment = self.call_value().single_dcdt();
        let sc_address = self.blockchain().get_sc_address();
        let original_token_data = self.blockchain().get_dcdt_token_data(
            &sc_address,
        ...
        let fractional_nonce = self.send().dcdt_nft_create(
            fractional_token,
    ...
    fn unfractionalize_nft(&self) {
        let fractional_payment = self.call_value().single_dcdt();
        let fractional_token_mapper = self.fractional_token();
