...
impl StaticVarApiImpl for DebugApi {
    ...
    fn set_call_value_rewa_handle(&self, handle: Self::BigIntHandle) {
        self.static_vars_cell.borrow_mut().call_value_rewa_handle = handle.get_raw_handle();
    }

    fn get_call_value_rewa_handle(&self) -> Self::BigIntHandle {
        use_raw_handle(self.static_vars_cell.borrow().call_value_rewa_handle)
    }

    fn set_call_value_multi_dcdt_handle(&self, handle: Self::ManagedBufferHandle) {
        self.static_vars_cell
            .borrow_mut()
            .call_value_multi_dcdt_handle = handle.get_raw_handle();
    }

    fn get_call_value_multi_dcdt_handle(&self) -> Self::ManagedBufferHandle {
        use_raw_handle(self.static_vars_cell.borrow().call_value_multi_dcdt_handle)
    }
}
