...
impl BigIntApi for DebugApi {
    ...
    fn bi_sign(&self, x: Self::BigIntHandle) -> drt_sc::api::Sign {
        let bi = self.bi_get(x);
        match bi.sign() {
            num_bigint::Sign::Minus => drt_sc::api::Sign::Minus,
            num_bigint::Sign::NoSign => drt_sc::api::Sign::NoSign,
            num_bigint::Sign::Plus => drt_sc::api::Sign::Plus,
        }
    }
