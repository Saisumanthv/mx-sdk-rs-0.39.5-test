...
use drt_sc::contract_base::{CallableContractBuilder, ContractAbiProvider};
...
impl BlockchainMock {
    ...
    {
        let multi_contract_config = drt_sc_meta::multi_contract_config::<Abi>(
            self.current_dir
        ...
        let contract_obj = if sub_contract.settings.external_view {
            contract_builder.new_contract_obj::<drt_sc::api::ExternalViewApi<DebugApi>>()
        } else {
