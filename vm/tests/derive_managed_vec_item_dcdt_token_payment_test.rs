...
use drt_sc::{
    ...
    types::{BigUint, DcdtTokenPayment, ManagedByteArray, ManagedType, TokenIdentifier},
};
...
pub struct ManagedStructWithToken<M: ManagedTypeApi> {
    pub token: drt_sc::types::DcdtTokenPayment<M>,
    pub num: u32,
...
fn struct_with_numbers_static() {
    assert_eq!(
        <ManagedStructWithToken<DebugApi> as drt_sc::types::ManagedVecItem>::PAYLOAD_SIZE,
        28
    );
    assert!(
        !<ManagedStructWithToken<DebugApi> as drt_sc::types::ManagedVecItem>::SKIPS_RESERIALIZATION
    );
}
...
fn struct_to_bytes_writer() {
    ...
    <ManagedStructWithToken<DebugApi> as drt_sc::types::ManagedVecItem>::to_byte_writer(
        &s,
        |bytes| {
            arr[0
                ..<ManagedStructWithToken<DebugApi> as drt_sc::types::ManagedVecItem>::PAYLOAD_SIZE]
...
fn struct_from_bytes_reader() {
    ...
    let struct_from_bytes =
        <ManagedStructWithToken<DebugApi> as drt_sc::types::ManagedVecItem>::from_byte_reader(
            |bytes| {
